pub mod transport;
pub mod parser;
pub mod secure_transport;

extern crate rusthacl;
extern crate xml;
extern crate byteorder;
extern crate rand;

// TODO: remove once all tests are done
extern crate ivyrust;


#[cfg(test)]
mod tests {
    use super::secure_transport::{SecurePprzTransport, StsParty, StsStage};
    use super::transport::PprzTransport;
    use super::parser;
    use super::parser::{PprzMsgClassID, PprzMsgBaseType};

    use std::fs::File;
    use std::sync::Arc;

    #[test]
    fn test_secure_transport_key_exchange() {
        let dictionary = Arc::new(get_dictionary_v2());

        // init A
        let mut party_a = SecurePprzTransport::new(StsParty::Initiator, 4); // the destination id = 4
        party_a.set_sender(0);
        party_a.my_private_key.set(&Q_A, &P_A).unwrap();
        party_a.their_public_key.set(&P_B).unwrap();
        party_a.dictionary = Some(dictionary.clone());
        party_a.set_msg_class(PprzMsgClassID::Telemetry); // party A reads telemetry messages


        // init B
        let mut party_b = SecurePprzTransport::new(StsParty::Responder, 0);
        party_b.set_sender(4); // sender id = 4
        party_b.my_private_key.set(&Q_B, &P_B).unwrap();
        party_b.their_public_key.set(&P_A).unwrap();
        party_b.dictionary = Some(dictionary.clone());
        party_b.set_msg_class(PprzMsgClassID::Datalink); // party B reads Datalink messages

        // we need to init to the proper stage
        assert!(party_b.construct_pprz_msg(&vec![]) == None);

        // 1. A generates an ephemeral (random) curve25519 key pair (Pae, Qae) and sends Pae.
        println!("A Stage: {:?}", party_a.get_stage());
        let msg1 = party_a.construct_pprz_msg(&vec![]);
        println!("A Stage: {:?}", party_a.get_stage());
        assert!(msg1 != None);
        let msg1 = msg1.unwrap();
        //println!("Msg1 = {:?}", msg1);

        // 2. send msg to B
        for byte in msg1 {
            party_b.parse_byte(byte);
        }
        // now we should have a message 2 ready
        println!("B Stage: {:?}", party_b.get_stage());
        let msg2 = party_b.construct_pprz_msg(&vec![]);
        println!("{:?}", party_b.last_error);
        println!("{}", party_b.last_error_message);
        println!("B Stage: {:?}", party_b.get_stage());
        assert!(msg2 != None);
        let msg2 = msg2.unwrap();
        //println!("Msg2 = {:?}", msg2);

        // 3. send msg2 to A
        for byte in msg2 {
            party_a.parse_byte(byte);
        }
        // now we should have message 3 ready, get it
        let msg3 = party_a.construct_pprz_msg(&vec![]);
        assert!(msg3 != None);
        assert!(*party_a.get_stage() == StsStage::CryptoOK);
        println!("A stage: {:?}", party_a.get_stage());
        let msg3 = msg3.unwrap();
        //println!("Msg3 = {:?}", msg3);

        // And send msg3 to B
        println!("B stage: {:?}", party_b.get_stage());
        for byte in msg3 {
            party_b.parse_byte(byte);
        }
        println!("B stage: {:?}", party_b.get_stage());
        println!("{:?}", party_b.last_error);
        println!("{}", party_b.last_error_message);
        assert!(*party_b.get_stage() == StsStage::CryptoOK);

        // we should have both parties properly exchange keys
        // lets test it!
        // B -> A
        let (_, buffer_of_payloads) = initialize_vectors_v2();
        let mut counter = 0;
        let buffer_len = buffer_of_payloads.len();
        for message in buffer_of_payloads {
            let tx = party_b.construct_pprz_msg(&message);
            assert!(tx != None);
            let tx = tx.unwrap();

            for byte in tx {
                if let Some(msg) = party_a.parse_byte(byte) {
                    assert_eq!(msg, message);
                    counter += 1;
                }
            }
        }
        assert_eq!(counter, buffer_len);

        // Move WP message
        let mut msg_move_wp = dictionary.find_msg_by_name(&"MOVE_WP").unwrap();
        msg_move_wp.set_destinaton(4);
        let msg_as_bytes = msg_move_wp.to_bytes();
        let tx = party_a.construct_pprz_msg(&msg_as_bytes);
        assert!(tx != None);
        let tx = tx.unwrap();

        for byte in tx {
            if let Some(msg) = party_b.parse_byte(byte) {
                assert_eq!(msg, msg_as_bytes);
            }
        }

        let mut msg_move_wp = dictionary.find_msg_by_name(&"PING").unwrap();
        msg_move_wp.set_destinaton(4);
        let msg_as_bytes = msg_move_wp.to_bytes();
        let tx = party_a.construct_pprz_msg(&msg_as_bytes);
        assert!(tx != None);
        let tx = tx.unwrap();

        for byte in tx {
            if let Some(msg) = party_b.parse_byte(byte) {
                assert_eq!(msg, msg_as_bytes);
            }
        }
    }


    use super::ivyrust;
    use std::{thread, time};
    //==========================
    // Test pprzlink ivy
    //==========================
    #[test]
    fn test_ivy_to_pprzlink() {
        // this test will bind callbacks to varius Ivy messages and then listen on thebus
        let dictionary = get_dictionary();

        ivyrust::ivy_init(String::from("RUST_IVY"), String::from("RUST_IVY Ready"));
        ivyrust::ivy_start(None);

        // spin the main loop
        let _ = thread::spawn(move || if let Err(e) = ivyrust::ivy_main_loop() {
            println!("Error in Ivy main loop: {}", e);
        } else {
            println!("ivy main loop finished finished");
        });
        // sleep so we can finish initialization
        thread::sleep(time::Duration::from_millis(1000));

        // find a message to be bound to IVY bus
        let msg = dictionary.find_msg_by_name("NPS_POS_LLH").unwrap();

        // lets bind a message
        let mut cb1 = ivyrust::IvyMessage::new();
        assert_eq!(
            true,
            cb1.ivy_bind_msg(ivyrust::IvyMessage::callback, msg.to_ivy_regexpr())
        );

        thread::sleep(time::Duration::from_millis(10000));
        // simply terminate
        println!("Test sucessful, terminating");
    }

    ///  run with ivyprobe '(.*)'
    #[test]
    fn test_pprzlink_to_ivy() {
        let (buffer_of_messages, _) = initialize_vectors();
        let dictionary = get_dictionary();

        ivyrust::ivy_init(String::from("RUST_IVY"), String::from("RUST_IVY Ready"));
        ivyrust::ivy_start(None);

        // spin the main loop
        let _ = thread::spawn(move || if let Err(e) = ivyrust::ivy_main_loop() {
            println!("Error in Ivy main loop: {}", e);
        } else {
            println!("ivy main loop finished finished");
        });
        // sleep so we can finish initialization
        thread::sleep(time::Duration::from_millis(1000));

        for message in buffer_of_messages {
            let mut payload = message.clone();
            let mut rx = PprzTransport::new();
            let mut msg_available = false;
            for b in message {
                if msg_available {
                    panic!("Too many bytes!");
                }
                msg_available = rx.parse_byte(b);
            }
            assert!(msg_available);


            let name = dictionary
                .get_msg_name(PprzMsgClassID::Telemetry, rx.buf[1])
                .unwrap();
            let mut msg = dictionary.find_msg_by_name(&name).unwrap();

            // update message fields with real values
            msg.update(&rx.buf);

            // send the message
            ivyrust::ivy_send_msg(msg.to_string().unwrap());

            // now get the byte representation of the payload
            let msg_as_bytes: Vec<u8> = msg.to_bytes();

            // drain first two bytes and the last two bytes (transport header and checksum)
            let len = payload.len();
            let payload: Vec<u8> = payload.drain(2..len - 2).collect();

            println!("MSG: {}", msg);

            assert_eq!(msg_as_bytes, payload);
        }

        // sleep a bit so we can finish sending messages
        thread::sleep(time::Duration::from_millis(1000));
        ivyrust::ivy_stop();
    }

    #[test]
    fn test_pprzlink_to_ivy_v2() {
        let (buffer_of_messages, _) = initialize_vectors_v2();
        let dictionary = get_dictionary_v2();

        ivyrust::ivy_init(String::from("RUST_IVY"), String::from("RUST_IVY Ready"));
        ivyrust::ivy_start(None);

        // spin the main loop
        let _ = thread::spawn(move || if let Err(e) = ivyrust::ivy_main_loop() {
            println!("Error in Ivy main loop: {}", e);
        } else {
            println!("ivy main loop finished finished");
        });
        // sleep so we can finish initialization
        thread::sleep(time::Duration::from_millis(1000));

        for message in buffer_of_messages {
            let mut payload = message.clone();
            let mut rx = PprzTransport::new();
            let mut msg_available = false;
            for b in message {
                if msg_available {
                    panic!("Too many bytes!");
                }
                msg_available = rx.parse_byte(b);
            }
            assert!(msg_available);

            let name = dictionary
                .get_msg_name(PprzMsgClassID::Telemetry, rx.buf[3]) // TODO: make this general
                .unwrap();
            let mut msg = dictionary.find_msg_by_name(&name).unwrap();

            // update message fields with real values
            msg.update(&rx.buf);

            // send the message
            ivyrust::ivy_send_msg(msg.to_string().unwrap());

            // now get the byte representation of the payload
            let msg_as_bytes: Vec<u8> = msg.to_bytes();

            // drain first two bytes and the last two bytes (transport header and checksum)
            let len = payload.len();
            let payload: Vec<u8> = payload.drain(2..len - 2).collect();

            println!("MSG: {}", msg);

            assert_eq!(msg_as_bytes, payload);
        }

        // sleep a bit so we can finish sending messages
        thread::sleep(time::Duration::from_millis(1000));
        ivyrust::ivy_stop();
    }

    //==========================
    // Test pprzlink parser
    //==========================
    fn get_dictionary() -> parser::PprzDictionary {
        let file = File::open("test/messages.xml").unwrap();
        let dictionary = parser::build_dictionary(file, parser::PprzProtocolVersion::ProtocolV1);
        assert_eq!(dictionary.classes.len(), 5);
        dictionary
    }

    fn get_dictionary_v2() -> parser::PprzDictionary {
        let file = File::open("test/messages.xml").unwrap();
        let dictionary = parser::build_dictionary(file, parser::PprzProtocolVersion::ProtocolV2);
        assert_eq!(dictionary.classes.len(), 5);
        dictionary
    }

    #[test]
    fn dictionary_find_msg_by_name() {
        let dictionary = get_dictionary();

        let name = String::from("PING");
        let msg = match dictionary.find_msg_by_name(&name) {
            Some(m) => m,
            None => panic!("No message found"),
        };

        println!("Msg: {}", msg);
        assert_eq!(msg.name, name);
    }

    #[test]
    fn dictionary_find_msg_by_name_v2() {
        let dictionary = get_dictionary_v2();

        let name = String::from("PING");
        let msg = match dictionary.find_msg_by_name(&name) {
            Some(m) => m,
            None => panic!("No message found"),
        };

        println!("Msg: {}", msg);
        assert_eq!(msg.name, name);
    }

    #[test]
    fn get_and_modify_msg_field_v2() {
        let dictionary = get_dictionary_v2();
        let mut msg = dictionary.find_msg_by_name(&"KEY_EXCHANGE_GCS").unwrap();
        println!("msg = {}", msg);
        println!("msg.id = {}", msg.id);
        msg.update_single_field("msg_type", PprzMsgBaseType::Uint8(1));

        let v = vec![1, 2, 3, 4, 5];
        msg.update_single_field("msg_data", PprzMsgBaseType::Uint8Arr(v));
        println!("msg = {}", msg);
    }

    #[test]
    fn dictionary_get_msgs() {
        let dictionary = get_dictionary();

        let id = parser::PprzMsgClassID::Datalink;
        let msgs = match dictionary.get_msgs(id) {
            Some(m) => m,
            None => panic!("No message class found"),
        };

        assert_eq!(msgs.messages.len(), 56);
    }

    #[test]
    fn dictionary_get_msgs_v2() {
        let dictionary = get_dictionary_v2();

        let id = parser::PprzMsgClassID::Datalink;
        let msgs = match dictionary.get_msgs(id) {
            Some(m) => m,
            None => panic!("No message class found"),
        };

        assert_eq!(msgs.messages.len(), 56);
    }

    #[test]
    fn dictionary_get_msg_name() {
        let dictionary = get_dictionary();

        let class_id = parser::PprzMsgClassID::Telemetry;
        let msg_id = 30;
        let name = match dictionary.get_msg_name(class_id, msg_id) {
            Some(n) => n,
            None => panic!("Message not found"),
        };

        assert_eq!(name, "DATALINK_REPORT");
    }

    #[test]
    fn dictionary_get_msg_name_v2() {
        let dictionary = get_dictionary();

        let class_id = parser::PprzMsgClassID::Telemetry;
        let msg_id = 30;
        let name = match dictionary.get_msg_name(class_id, msg_id) {
            Some(n) => n,
            None => panic!("Message not found"),
        };

        assert_eq!(name, "DATALINK_REPORT");
    }

    #[test]
    fn dictionary_get_msg_id() {
        let dictionary = get_dictionary();

        let class_id = parser::PprzMsgClassID::Telemetry;
        let name = "GPS_SOL";
        let id = match dictionary.get_msg_id(class_id, name) {
            Some(i) => i,
            None => panic!("No message found"),
        };

        assert_eq!(id, 17);
    }

    #[test]
    fn dictionary_get_msg_id_v2() {
        let dictionary = get_dictionary_v2();

        let class_id = parser::PprzMsgClassID::Telemetry;
        let name = "GPS_SOL";
        let id = match dictionary.get_msg_id(class_id, name) {
            Some(i) => i,
            None => panic!("No message found"),
        };

        assert_eq!(id, 17);
    }


    #[test]
    fn dictionary_get_msg_fields() {
        let dictionary = get_dictionary();

        let class_id = parser::PprzMsgClassID::Telemetry;
        let name = "GPS";
        let fields = match dictionary.get_msg_fields(class_id, name) {
            Some(f) => f,
            None => panic!("Message not found"),
        };

        assert_eq!(fields.len(), 11);
    }

    #[test]
    fn dictionary_get_msg_fields_v2() {
        let dictionary = get_dictionary_v2();

        let class_id = parser::PprzMsgClassID::Telemetry;
        let name = "GPS";
        let fields = match dictionary.get_msg_fields(class_id, name) {
            Some(f) => f,
            None => panic!("Message not found"),
        };

        assert_eq!(fields.len(), 11);
    }

    #[test]
    fn parse_xml_file() {
        let file = File::open("test/messages.xml").unwrap();
        let dictionary = parser::build_dictionary(file, parser::PprzProtocolVersion::ProtocolV1);
        // 5 classes in messages.xml
        assert_eq!(dictionary.classes.len(), 5);
    }

    #[test]
    fn parse_xml_file_v2() {
        let file = File::open("test/messages.xml").unwrap();
        let dictionary = parser::build_dictionary(file, parser::PprzProtocolVersion::ProtocolV2);
        // 5 classes in messages.xml
        assert_eq!(dictionary.classes.len(), 5);
    }

    //==========================
    // Test pprzlink tramsport
    //==========================
    #[test]
    /// Decode a single message
    fn decode_message() {
        let buf: Vec<u8> = vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                                0x39, 0x1e];
        let mut rx = PprzTransport::new();
        let mut msg_available = false;
        for b in buf {
            println!("b=0x{:x}", b);
            msg_available = rx.parse_byte(b);
            if msg_available {
                println!("message available");
            }
        }
        assert!(msg_available);
    }

    #[test]
    /// Decode a single message
    fn decode_message_v2() {
        let buf: Vec<u8> = vec![0x99, 0x1b, 0x4, 0x0, 0x1, 0x21, 0x69, 0x18, 0x51, 0x18, 0x7f,
                                0x18, 0xa2, 0x6, 0x55, 0x6, 0xb8, 0x7, 0xf6, 0x2, 0x1b, 0x66,
                                0xa6, 0x8a, 0x42, 0x6f, 0xec];
        let mut rx = PprzTransport::new();
        let mut msg_available = false;
        for b in buf {
            println!("b=0x{:x}", b);
            msg_available = rx.parse_byte(b);
            if msg_available {
                println!("message available");
            }
        }
        assert!(msg_available);
    }

    #[test]
    /// Decode 138 messages
    fn decode_messages() {
        let (buffer_of_messages, _) = initialize_vectors();

        let mut rx = PprzTransport::new();
        let mut n_msg = 0;

        let mut buf: Vec<u8> = vec![];
        // populate buffer
        for msg in buffer_of_messages {
            for byte in msg {
                buf.push(byte);
            }
        }

        for b in buf {
            let msg_available = rx.parse_byte(b);
            if msg_available {
                n_msg += 1;
            }
        }
        assert!(n_msg == 138);
    }

    #[test]
    /// Decode 138 messages
    fn decode_messages_v2() {
        let (buffer_of_messages, _) = initialize_vectors_v2();

        let mut rx = PprzTransport::new();
        let mut n_msg = 0;

        let mut buf: Vec<u8> = vec![];
        // populate buffer
        for msg in buffer_of_messages {
            for byte in msg {
                buf.push(byte);
            }
        }

        for b in buf {
            let msg_available = rx.parse_byte(b);
            if msg_available {
                n_msg += 1;
            }
        }
        assert!(n_msg == 60);
    }


    #[test]
    /// encode a single message and verify
    fn encode_message() {
        let buf: Vec<u8> = vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                                0x39, 0x1e];
        let payload: Vec<u8> = vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0];
        let mut tx = PprzTransport::new();

        tx.construct_pprz_msg(&payload);

        // verify
        for idx in 0..buf.len() {
            println!(
                "buf[{}] = 0x{:x}; tx.buf[{}] = 0x{:x}",
                idx,
                buf[idx],
                idx,
                tx.buf[idx]
            );
            assert!(buf[idx] == tx.buf[idx]);
        }
    }

    #[test]
    /// encode a single message and verify
    fn encode_message_v2() {
        let buf: Vec<u8> = vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x31, 0x11, 0xcc, 0x5, 0x0, 0x0, 0x34,
                                0xf5, 0xff, 0xff, 0x0, 0x5, 0x0, 0x0, 0x59, 0x76];
        let payload: Vec<u8> = vec![0x4, 0x0, 0x1, 0x31, 0x11, 0xcc, 0x5, 0x0, 0x0, 0x34, 0xf5,
                                    0xff, 0xff, 0x0, 0x5, 0x0, 0x0];
        let mut tx = PprzTransport::new();

        tx.construct_pprz_msg(&payload);

        // verify
        for idx in 0..buf.len() {
            println!(
                "buf[{}] = 0x{:x}; tx.buf[{}] = 0x{:x}",
                idx,
                buf[idx],
                idx,
                tx.buf[idx]
            );
            assert!(buf[idx] == tx.buf[idx]);
        }
    }


    #[test]
    /// encode 128 messages and verify
    fn encode_messages() {
        let (buffer_of_messages, buffer_of_payloads) = initialize_vectors();

        assert_eq!(buffer_of_payloads.len(), buffer_of_messages.len());

        let mut tx = PprzTransport::new();

        for idx in 0..buffer_of_payloads.len() {
            let data: Vec<u8> = buffer_of_payloads[idx].clone();
            let original_msg: Vec<u8> = buffer_of_messages[idx].clone();

            tx.construct_pprz_msg(&data);

            // verify
            for idx in 0..original_msg.len() {
                println!(
                    "orginal.buf[{}] = 0x{:x}; tx.buf[{}] = 0x{:x}",
                    idx,
                    original_msg[idx],
                    idx,
                    tx.buf[idx]
                );
                assert!(original_msg[idx] == tx.buf[idx]);
            }
        }
    }

    #[test]
    /// encode 60 messages and verify
    fn encode_messages_v2() {
        let (buffer_of_messages, buffer_of_payloads) = initialize_vectors_v2();

        assert_eq!(buffer_of_payloads.len(), buffer_of_messages.len());

        let mut tx = PprzTransport::new();

        for idx in 0..buffer_of_payloads.len() {
            let data: Vec<u8> = buffer_of_payloads[idx].clone();
            let original_msg: Vec<u8> = buffer_of_messages[idx].clone();

            tx.construct_pprz_msg(&data);

            // verify
            for idx in 0..original_msg.len() {
                println!(
                    "orginal.buf[{}] = 0x{:x}; tx.buf[{}] = 0x{:x}",
                    idx,
                    original_msg[idx],
                    idx,
                    tx.buf[idx]
                );
                assert!(original_msg[idx] == tx.buf[idx]);
            }
        }
    }

    fn initialize_vectors() -> (Vec<Vec<u8>>, Vec<Vec<u8>>) {
        let buffer_of_payloads: Vec<Vec<u8>> =
            vec![vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xeb, 0x1, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x23, 0x8, 0xdd, 0x55, 0xcd, 0x48, 0xd2, 0x44, 0x8d, 0x4a, 0x0, 0x20,
                      0xe0, 0x44, 0xc],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0xd7, 0x43,
                      0xf6, 0x43],
                 vec![0x1, 0x23, 0x9, 0xe3, 0xb9, 0xcc, 0x48, 0xe0, 0x44, 0x8d, 0x4a, 0x0, 0x20,
                      0xe0, 0x44, 0xc],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x69, 0x6, 0x4c, 0x4, 0xdc, 0x5, 0xdc, 0x5, 0xdc, 0x5, 0xdc, 0x5,
                      0xdc, 0x5],
                 vec![0x1, 0x69, 0x6, 0x4c, 0x4, 0xdc, 0x5, 0xdc, 0x5, 0xdc, 0x5, 0xdc, 0x5,
                      0xdc, 0x5],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x67, 0x2, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x23, 0xa, 0x8a, 0xcd, 0xcb, 0x48, 0xf1, 0x38, 0x8d, 0x4a, 0x0, 0xc0,
                      0xc1, 0x44, 0xc],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0xd7, 0xc3,
                      0xf6, 0x43],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xee, 0x1, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x1e, 0x81, 0x1, 0x9, 0x0, 0x0, 0x0, 0xe2, 0x2, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x23, 0xb, 0x46, 0xcf, 0xcb, 0x48, 0x7d, 0x58, 0x8d, 0x4a, 0x0, 0xc0,
                      0xc1, 0x44, 0xc],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x2, 0x10, 0x7b, 0x41, 0xdb, 0x42, 0xbb, 0x1e, 0xf1, 0x19, 0x4, 0xce,
                      0x1c, 0xb8, 0xff, 0x9f, 0xd5, 0x8b],
                 vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x1f, 0x45, 0x0, 0x0, 0x80, 0x3f],
                 vec![0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0xd7, 0x43,
                      0xf7, 0x43],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x23, 0xc, 0x43, 0xd0, 0xcd, 0x48, 0xea, 0x57, 0x8d, 0x4a, 0x0, 0xc0,
                      0xc1, 0x44, 0xc],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x67, 0x2, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x67, 0x2, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xef, 0x1, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x23, 0xd, 0xdd, 0xa6, 0xcd, 0x48, 0x92, 0x38, 0x8d, 0x4a, 0x0, 0xc0,
                      0xc1, 0x44, 0xc],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x9, 0x62, 0x62, 0x6, 0x0, 0x5, 0xa5, 0x46, 0x0, 0xc, 0x0, 0xc0, 0xa8,
                      0x44],
                 vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0xb, 0x2, 0x0, 0x0, 0x0, 0x0, 0x6],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0xd7, 0xc3,
                      0xf7, 0x43],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x23, 0xe, 0x53, 0x9e, 0xcc, 0x48, 0x88, 0x47, 0x8d, 0x4a, 0x0, 0x80,
                      0xaf, 0x44, 0xc],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x1f, 0x46, 0xcd, 0xcc, 0xcc, 0x3d],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0],
                 vec![0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x80, 0x41],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0x23, 0xf, 0xad, 0xdc, 0xcc, 0x48, 0x46, 0x47, 0x8d, 0x4a, 0x0, 0x60,
                      0xa8, 0x44, 0xc],
                 vec![0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xf0, 0x1, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]];

        let buffer_of_messages: Vec<Vec<u8>> =
            vec![vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e], // NAVIGATION
                 vec![0x99, 0x15, 0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xeb,
                      0x1, 0x0, 0x0, 0x0, 0x0, 0xf, 0xd9], // BATTERY
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9], // GPS LLA
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x14, 0x1, 0x23, 0x8, 0xdd, 0x55, 0xcd, 0x48, 0xd2, 0x44, 0x8d, 0x4a,
                      0x0, 0x20, 0xe0, 0x44, 0xc, 0xc4, 0x82],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x22, 0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x3a, 0xd7],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x11, 0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x7d, 0xfa],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x14, 0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0xd7, 0x43, 0xf6, 0x43, 0x8f, 0x61],
                 vec![0x99, 0x14, 0x1, 0x23, 0x9, 0xe3, 0xb9, 0xcc, 0x48, 0xe0, 0x44, 0x8d, 0x4a,
                      0x0, 0x20, 0xe0, 0x44, 0xc, 0x3c, 0x1],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x13, 0x1, 0x69, 0x6, 0x4c, 0x4, 0xdc, 0x5, 0xdc, 0x5, 0xdc, 0x5,
                      0xdc, 0x5, 0xdc, 0x5, 0x38, 0x4c],
                 vec![0x99, 0x13, 0x1, 0x69, 0x6, 0x4c, 0x4, 0xdc, 0x5, 0xdc, 0x5, 0xdc, 0x5,
                      0xdc, 0x5, 0xdc, 0x5, 0x38, 0x4c],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0xf, 0x1, 0x67, 0x2, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7a,
                      0xde],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x11, 0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x7d, 0xfa],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x14, 0x1, 0x23, 0xa, 0x8a, 0xcd, 0xcb, 0x48, 0xf1, 0x38, 0x8d, 0x4a,
                      0x0, 0xc0, 0xc1, 0x44, 0xc, 0x7d, 0xcb],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x22, 0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x3a, 0xd7],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x11, 0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x7d, 0xfa],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x14, 0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0xd7, 0xc3, 0xf6, 0x43, 0xf, 0xe1],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x15, 0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xee,
                      0x1, 0x0, 0x0, 0x0, 0x0, 0x12, 0xeb],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x11, 0x1, 0x1e, 0x81, 0x1, 0x9, 0x0, 0x0, 0x0, 0xe2, 0x2, 0x0, 0x0,
                      0x0, 0x9f, 0xbb],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x14, 0x1, 0x23, 0xb, 0x46, 0xcf, 0xcb, 0x48, 0x7d, 0x58, 0x8d, 0x4a,
                      0x0, 0xc0, 0xc1, 0x44, 0xc, 0xe8, 0x69],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x17, 0x1, 0x2, 0x10, 0x7b, 0x41, 0xdb, 0x42, 0xbb, 0x1e, 0xf1, 0x19,
                      0x4, 0xce, 0x1c, 0xb8, 0xff, 0x9f, 0xd5, 0x8b, 0x8a, 0x4f],
                 vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0xb, 0x1, 0x1f, 0x45, 0x0, 0x0, 0x80, 0x3f, 0x2f, 0xb1],
                 vec![0x99, 0x11, 0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x7d, 0xfa],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x14, 0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0xd7, 0x43, 0xf7, 0x43, 0x90, 0x63],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x14, 0x1, 0x23, 0xc, 0x43, 0xd0, 0xcd, 0x48, 0xea, 0x57, 0x8d, 0x4a,
                      0x0, 0xc0, 0xc1, 0x44, 0xc, 0x55, 0x3f],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0xf, 0x1, 0x67, 0x2, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7a,
                      0xde],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0xf, 0x1, 0x67, 0x2, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7a,
                      0xde],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x15, 0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xef,
                      0x1, 0x0, 0x0, 0x0, 0x0, 0x13, 0xf1],
                 vec![0x99, 0x14, 0x1, 0x23, 0xd, 0xdd, 0xa6, 0xcd, 0x48, 0x92, 0x38, 0x8d, 0x4a,
                      0x0, 0xc0, 0xc1, 0x44, 0xc, 0x4f, 0x17],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x13, 0x1, 0x9, 0x62, 0x62, 0x6, 0x0, 0x5, 0xa5, 0x46, 0x0, 0xc, 0x0,
                      0xc0, 0xa8, 0x44, 0x8f, 0xe0],
                 vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x22, 0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x3a, 0xd7],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x11, 0x1, 0x66, 0x5, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x7d, 0xfa],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0xc, 0x1, 0xb, 0x2, 0x0, 0x0, 0x0, 0x0, 0x6, 0x20, 0xd3],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x14, 0x1, 0xf6, 0xf, 0x0, 0xf8, 0x29, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0xd7, 0xc3, 0xf7, 0x43, 0x10, 0xe3],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x14, 0x1, 0x23, 0xe, 0x53, 0x9e, 0xcc, 0x48, 0x88, 0x47, 0x8d, 0x4a,
                      0x0, 0x80, 0xaf, 0x44, 0xc, 0x70, 0xa0],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x24, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x60, 0xe9],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0xb, 0x1, 0x1f, 0x46, 0xcd, 0xcc, 0xcc, 0x3d, 0x13, 0xe4],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x1a, 0x1, 0xa, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x9f, 0x40, 0x7d, 0x44, 0x0, 0x0, 0xc7, 0x1e],
                 vec![0x99, 0x26, 0x1, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xaf, 0x44,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80, 0x41, 0xeb, 0xdf],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x14, 0x1, 0x23, 0xf, 0xad, 0xdc, 0xcc, 0x48, 0x46, 0x47, 0x8d, 0x4a,
                      0x0, 0x60, 0xa8, 0x44, 0xc, 0xa0, 0x41],
                 vec![0x99, 0x12, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x19, 0x6a],
                 vec![0x99, 0x15, 0x1, 0xc, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xf0,
                      0x1, 0x0, 0x0, 0x0, 0x0, 0x14, 0xf7],
                 vec![0x99, 0xe, 0x1, 0x2a, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x39, 0x1e]];

        (buffer_of_messages, buffer_of_payloads)
    }

    fn initialize_vectors_v2() -> (Vec<Vec<u8>>, Vec<Vec<u8>>) {
        let buffer_of_payloads: Vec<Vec<u8>> =
            vec![vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xe7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x44, 0x0],
                 vec![0x4, 0x0, 0x1, 0xf8, 0x2, 0x3, 0x4, 0x0, 0x0, 0x0, 0x0, 0x96, 0xc0, 0x0,
                      0x0, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x6,
                      0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0,
                      0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x4, 0x6, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x6, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xd0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x31, 0x11, 0xcc, 0x5, 0x0, 0x0, 0x34, 0xf5, 0xff, 0xff,
                      0x0, 0x5, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2,
                      0x3, 0xf2, 0x3],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda],
                 vec![0x4, 0x0, 0x1, 0x21, 0x69, 0x18, 0x51, 0x18, 0x7f, 0x18, 0xa2, 0x6, 0x55,
                      0x6, 0xb8, 0x7, 0xf6, 0x2, 0x1b, 0x66, 0xa6, 0x8a, 0x42],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2,
                      0x3, 0xf2, 0x3],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x1f, 0xb, 0x0, 0x0, 0x20, 0x41],
                 vec![0x4, 0x0, 0x1, 0xf8, 0x2, 0x2, 0x4, 0x0, 0x0, 0x0, 0x0, 0x96, 0xc0, 0x0,
                      0x0, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x6,
                      0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0,
                      0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x4, 0x6, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x6, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xe7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x45, 0x0],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x9f, 0x45, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x2, 0x0],
                 vec![0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2,
                      0x3, 0xf2, 0x3],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x31, 0x0, 0x3d, 0x29, 0x0, 0x0, 0xc5, 0x2a, 0x0, 0x0, 0x0,
                      0x5, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x2, 0x10, 0x23, 0x10, 0x2a, 0xd9, 0xe3, 0x41, 0x6f, 0xdf,
                      0x7e, 0xfa, 0x24, 0xc9, 0x84, 0xb0, 0xc9, 0xd1],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda],
                 vec![0x4, 0x0, 0x1, 0x21, 0x69, 0x18, 0x1b, 0x18, 0xc2, 0x18, 0xa3, 0x6, 0x56,
                      0x6, 0xba, 0x7, 0xf6, 0x2, 0x1b, 0x66, 0xa6, 0x8c, 0x42],
                 vec![0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0],
                 vec![0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0xf2,
                      0x3, 0xf2, 0x3],
                 vec![0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0],
                 vec![0x4, 0x0, 0x1, 0xf8, 0x2, 0x1, 0x4, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x6, 0x0,
                      0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0,
                      0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x4, 0x6, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x6, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0]];

        let buffer_of_messages: Vec<Vec<u8>> =
            vec![vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x19, 0x4, 0x0, 0x1, 0xe7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x44, 0x0, 0x4b, 0x6b],
                 vec![0x99, 0x57, 0x4, 0x0, 0x1, 0xf8, 0x02, 0x03, 0x4, 0x0, 0x0, 0x0, 0x0, 0x96,
                      0xc0, 0x0, 0x0, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x6, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41,
                      0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x4, 0x6,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x6, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x9, 0xcd],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x24, 0x4, 0x0, 0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x40, 0xdd],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0xf, 0x4, 0x0, 0x1, 0xd0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0xe5,
                      0x6a],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x31, 0x11, 0xcc, 0x5, 0x0, 0x0, 0x34, 0xf5,
                      0xff, 0xff, 0x0, 0x5, 0x0, 0x0, 0x59, 0x76],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0xac, 0xe3],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3,
                      0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0x47, 0xf9],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x14, 0x4, 0x0, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x1f, 0xf0],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0xac, 0xe3],
                 vec![0x99, 0x1b, 0x4, 0x0, 0x1, 0x21, 0x69, 0x18, 0x51, 0x18, 0x7f, 0x18, 0xa2,
                      0x6, 0x55, 0x6, 0xb8, 0x7, 0xf6, 0x2, 0x1b, 0x66, 0xa6, 0x8a, 0x42, 0x6f,
                      0xec],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0x26, 0x4, 0x0, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x66, 0xff],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3,
                      0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0x47, 0xf9],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0xd, 0x4, 0x0, 0x1, 0x1f, 0xb, 0x0, 0x0, 0x20, 0x41, 0x9d, 0x1f],
                 vec![0x99, 0x57, 0x4, 0x0, 0x1, 0xf8, 0x2, 0x2, 0x4, 0x0, 0x0, 0x0, 0x0, 0x96,
                      0xc0, 0x0, 0x0, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x6, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41,
                      0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x4, 0x6,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x6, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x8, 0x7f],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x24, 0x4, 0x0, 0x1, 0x17, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x40, 0xdd],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0x19, 0x4, 0x0, 0x1, 0xe7, 0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x45, 0x0, 0x4c, 0x6d],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x17, 0x4, 0x0, 0x1, 0x9f, 0x45, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x2, 0x0, 0x2, 0x28],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0xac, 0xe3],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3,
                      0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0x47, 0xf9],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x31, 0x0, 0x3d, 0x29, 0x0, 0x0, 0xc5, 0x2a,
                      0x0, 0x0, 0x0, 0x5, 0x0, 0x0, 0xa5, 0x77],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0x19, 0x4, 0x0, 0x1, 0x2, 0x10, 0x23, 0x10, 0x2a, 0xd9, 0xe3, 0x41,
                      0x6f, 0xdf, 0x7e, 0xfa, 0x24, 0xc9, 0x84, 0xb0, 0xc9, 0xd1, 0xb, 0xdc],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x2c, 0x4, 0x0, 0x1, 0xc6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xf7, 0x70],
                 vec![0x99, 0x14, 0x4, 0x0, 0x1, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x1f, 0xf0],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x70, 0x6, 0x80, 0xda, 0x80, 0xda, 0x80, 0xda,
                      0x80, 0xda, 0x80, 0xda, 0x80, 0xda, 0xac, 0xe3],
                 vec![0x99, 0x1b, 0x4, 0x0, 0x1, 0x21, 0x69, 0x18, 0x1b, 0x18, 0xc2, 0x18, 0xa3,
                      0x6, 0x56, 0x6, 0xba, 0x7, 0xf6, 0x2, 0x1b, 0x66, 0xa6, 0x8c, 0x42, 0x82,
                      0x71],
                 vec![0x99, 0x41, 0x4, 0x0, 0x1, 0x9b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0xe1, 0xb],
                 vec![0x99, 0x10, 0x4, 0x0, 0x1, 0x19, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x2e, 0xeb],
                 vec![0x99, 0x26, 0x4, 0x0, 0x1, 0x3b, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x66, 0xff],
                 vec![0x99, 0x15, 0x4, 0x0, 0x1, 0x69, 0x6, 0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3,
                      0xf2, 0x3, 0xf2, 0x3, 0xf2, 0x3, 0x47, 0xf9],
                 vec![0x99, 0x42, 0x4, 0x0, 0x1, 0x93, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0xda, 0x53],
                 vec![0x99, 0x57, 0x4, 0x0, 0x1, 0xf8, 0x2, 0x1, 0x4, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x0, 0x0, 0x6, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
                      0x0, 0x6, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41,
                      0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x0, 0x0, 0xc8, 0x41, 0x4, 0x6,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0x6, 0x0,
                      0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xb1, 0xc1]];

        (buffer_of_messages, buffer_of_payloads)
    }

    const Q_A: [u8; 32] = [0xbc, 0xc9, 0xa5, 0x89, 0x7, 0x2b, 0x3c, 0x71, 0xbe, 0x86, 0x11, 0x1d,
                           0x85, 0x31, 0xb9, 0xbb, 0xf4, 0x29, 0x64, 0x9a, 0x7, 0xf1, 0xdb, 0x87,
                           0x5a, 0xa8, 0xf9, 0x49, 0x75, 0x8e, 0x3a, 0xd8];
    const P_A: [u8; 32] = [0xbe, 0x7e, 0xcf, 0x70, 0x20, 0x9f, 0x26, 0xe5, 0x2f, 0xa8, 0x89, 0x85,
                           0x14, 0x3, 0xe1, 0xef, 0x2a, 0x1b, 0x2f, 0xd8, 0xe3, 0x16, 0xd9, 0xa6,
                           0x2, 0x6c, 0xa5, 0xa6, 0xdd, 0x41, 0xa5, 0x25];
    const Q_B: [u8; 32] = [0x13, 0x4b, 0x63, 0x9e, 0x68, 0x0, 0x9c, 0x72, 0x8d, 0xb3, 0x64, 0xa0,
                           0xcd, 0xa3, 0xf3, 0x2f, 0xb5, 0x4d, 0x23, 0x8, 0x7f, 0x33, 0x2c, 0x79,
                           0x9f, 0xcd, 0x5f, 0x7d, 0x49, 0xa8, 0x25, 0xb5];
    const P_B: [u8; 32] = [0x6c, 0x62, 0x58, 0xf3, 0x59, 0xb7, 0x94, 0xae, 0xa, 0xdf, 0xb2, 0x16,
                           0x2b, 0xd6, 0x3e, 0x4a, 0xd9, 0xed, 0xd9, 0xaa, 0xd8, 0xdd, 0x4a, 0x4e,
                           0xae, 0xe2, 0x96, 0x7f, 0x83, 0x72, 0xf4, 0x85];
}
